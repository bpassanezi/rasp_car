...
int main( int argc, char** argv )
{
	...
	{		
		...
		cv::Rect rect;
		rect = printSQR(entrada, quadrado);
		number = printNumber(ind, mnist, entrada, rect);
        ...
        for (int i=0; i<rect.width; i++){
            for (int j=0; j<rect.height; j++){
                insideSQR(i,j)[0] = entrada(i+rect.x,j+rect.y)[0];
				insideSQR(i,j)[1] = entrada(i+rect.x,j+rect.y)[1];
				insideSQR(i,j)[2] = entrada(i+rect.x,j+rect.y)[2];
            }
        }
		if (rect.width != 0){
			resize(insideSQR, insideSQR, Size(120, 120));
			cv::putText(insideSQR, std::to_string(number), 
			Point(60, 60), FONT_HERSHEY_SCRIPT_SIMPLEX, 2, Scalar(255,0,0), 1, 8);
	
			tela_com_camera = grudaH(entrada, insideSQR);
		}
		else{
