...
int printNumber(CvKNearest& ind, MNIST& mnist, Mat_<COR>& img, Rect rec){
	...
	cv::putText(img, std::to_string((int)result(0,0)), 
		Point(rec.y + 2*rec.width, rec.x + 2*rec.width), FONT_HERSHEY_SCRIPT_SIMPLEX, 0.5, Scalar(255,0,0), 1, 8);
...
int main( int argc, char** argv )
{
	...
	{		
        ...
        Mat_<COR> insideSQR(rect.width,rect.height);
		Mat_<COR> tela_com_camera;

        #pragma omp parallel for	
        for (int i=0; i<rect.width; i++){
            for (int j=0; j<rect.height; j++){
                insideSQR(i,j)[0] = src(i+rect.x,j+rect.y)[0];
				insideSQR(i,j)[1] = src(i+rect.x,j+rect.y)[1];
				insideSQR(i,j)[2] = src(i+rect.x,j+rect.y)[2];
            }
        }
		if (rect.width != 0){
			resize(insideSQR, insideSQR, Size(120, 120));
			tela_com_camera = grudaH(src, insideSQR);
		}
		else{
			tela_com_camera = src;
		}
		...
		imshow("Template Matching - Machine Learning Video", tela_com_camera);
